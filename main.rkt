...
;(define-struct birb (x y v-y shape))
(define-struct bird-info (x y v-y))
(define-struct state (bird pipes gravity momentum scroll-speed score))
(define initial-score 0)
;(define (get-initial-state bird)
;  (make-state(bird 5 10))
;  )
...
(define-struct pipe-info (gap-y gap-x))
(define (pipes-init gap-y gap-x)
  (list (make-pipe-info gap-y gap-x))
  )
...
(define (get-initial-state)
  (make-state (make-bird-info bird-x bird-initial-y 0) (pipes-init (+ added-number (random random-threshold)) scene-width) initial-gravity initial-momentum initial-scroll-speed initial-score)
  )
...
(define (next-state-bird bird gravity)
   (match-let ([(bird-info _ y v-y) bird])
     (struct-copy bird-info bird [v-y (+ v-y gravity)] [y (+ y v-y)])
     )
  )
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([ (bird-info _ _ v-y) bird])
    (struct-copy bird-info bird [v-y (* -1 momentum)])
    )
  )
...
(define (change current-state pressed-key)
  (match-let ([ (state bird _ _ _ _ _) current-state])
    ( cond [(key=? pressed-key " ") (struct-copy state current-state [bird (next-state-bird-onspace (state-bird current-state) (state-momentum current-state))])]
           [else current-state]
           )
    )
  )
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (pipe) (match-let ([(pipe-info _ gap-x) pipe]) (struct-copy pipe-info pipe [gap-x (- gap-x scroll-speed)]))) pipes)
  )
...
(define (clean-pipes pipes)
  (filter (lambda(pipe) (>= (pipe-info-gap-x pipe) 0)) pipes)
  )
...
(define (check-empty pipes)
  (if (null? pipes)
      (pipes-init (+ added-number (random random-threshold)) scene-width)
      pipes
      )
  )
  
;(define (add-more-pipes pipes) 
 ; (let add-helper ([curr-pipes (check-empty pipes)] [pipes-to-add (- no-pipes (length (check-empty pipes)))] )
  ;   (if (zero? pipes-to-add)
   ;       curr-pipes
    ;      (add-helper (append curr-pipes (pipes-init (+ added-number (random random-threshold)) (+ (pipe-info-gap-x (last-pipe curr-pipes)) pipe-gap pipe-width))) (sub1 pipes-to-add) )
     ;     )
      ;)
 ; )
(define (add-more-pipes pipes)
  (if (null? pipes)
   (pipes-init (+ added-number (random random-threshold)) scene-width) 
   (if (< (length pipes) 6)
      (append pipes (list (make-pipe-info (+ added-number (random random-threshold)) (+ (get-pipe-x (last-pipe pipes)) pipe-gap pipe-width))))
      pipes
      )
  )
  )
...
(define (check-ground-collision bird)
 (>= (+ bird-height (bird-info-y bird)) ground-y)      
 )
   
   
(define birdo (make-bird-info 10 330 0))
(check-ground-collision birdo)
; invalid-state?
...
(define (invalid-state? current-state)
  (match-let ([ (state bird pipes _ _ _ _) current-state])
  (cond [(check-ground-collision bird) #t]
        [(check-pipe-collisions bird pipes) #t]
        [else #f]
        )
    )
  )
...
(define (make-left-point-upper-pipe pipe)
  (make-posn (pipe-info-gap-x pipe) 0)
  )
(define (make-right-point-upper-pipe pipe)
  (make-posn (+ pipe-width (pipe-info-gap-x pipe)) (pipe-info-gap-y pipe))
  )
(define (make-left-point-lower-pipe pipe)
  (make-posn (pipe-info-gap-x pipe) (+ pipe-self-gap (pipe-info-gap-y pipe)))
  )
(define (make-right-point-lower-pipe pipe)
  (make-posn (+ pipe-width (pipe-info-gap-x pipe)) ground-y)
  )
(define (make-left-point-bird bird)
  (make-posn (bird-info-x bird) (bird-info-y bird))
  )
(define (make-right-point-bird bird)
  (make-posn (+ bird-width (bird-info-x bird)) (+ bird-height (bird-info-y bird)))
  )
(define (check-pipe-collisions bird pipes)
  (if (null? pipes)
      #f
      (or (or (check-collision-rectangles (make-left-point-upper-pipe (first pipes)) (make-right-point-upper-pipe (first pipes)) (make-left-point-bird bird) (make-right-point-bird bird))
              (check-collision-rectangles (make-left-point-lower-pipe (first pipes)) (make-right-point-lower-pipe (first pipes)) (make-left-point-bird bird) (make-right-point-bird bird))
          ) (check-pipe-collisions bird (rest pipes))
           )
      )
  )
...
(define (next-state in-state)
  (match-let ([(state bird pipes gravity _ scroll-speed score) in-state])
    (struct-copy state in-state [bird (next-state-bird bird gravity )] [pipes (next-state-pipes  pipes scroll-speed)] [score (+ score 0.1)])
    )
  ) 
...
(define bird-graphic (rectangle bird-width bird-height  "solid" "yellow"))
...
(define bird-center-x (quotient bird-width 2))
(define bird-center-y (quotient bird-height 2))
(define pipe-graphic (rectangle pipe-width pipe-height "solid" "green"))
(define gap-graphic (rectangle pipe-width pipe-self-gap "solid" "white"))
(define pipe-center-x (quotient pipe-width 2))
(define pipe-center-y (quotient pipe-self-gap 2))
(define ground-center-x (quotient scene-width 2))
(define ground-center-y (quotient ground-height 2))
(define lower-pipe-y (- scene-height pipe-self-gap))
...
(define (draw-frame curr-state)
  (match-let ([ (state bird pipes _ _ _ score) curr-state] )
   (place-image bird-graphic (+ bird-center-x (bird-info-x bird)) (+ bird-center-y (bird-info-y bird)) (place-image ground ground-center-x (+ ground-center-y ground-y) (place-pipes pipes initial-scene))))
  )
...
(define (place-pipes pipes scene)
   (foldl (lambda(pipe acc) (place-image/align (rectangle pipe-width (pipe-info-gap-y pipe) "solid" "green") (get-pipe-x pipe) (pipe-info-gap-y pipe) "left" "bottom" (place-image/align (rectangle pipe-width (- lower-pipe-y (pipe-info-gap-y pipe)) "solid" "green") (pipe-info-gap-x pipe) (+ pipe-self-gap (pipe-info-gap-y pipe)) "left" "top" acc))
           )
         scene pipes)
  )
; Bonus
